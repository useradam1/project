...
def ClearTextures2DFromFrameBuffer(fbo: uint32, num_textures: int) -> None:
    # Привязываем буфер кадров
    glBindFramebuffer(GL_FRAMEBUFFER, fbo)
    
    # Отключаем текстуры от цветовых вложений
    for i in range(num_textures):
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, 0, 0)
    
    # Отвязываем буфер кадров
...
def UpdateSizeFrameBuffer(rbo: uint32, width: int, height: int) -> None:
