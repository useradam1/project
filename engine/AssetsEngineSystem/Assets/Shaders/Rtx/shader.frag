...
layout(std430, binding = 1) buffer Transforms {Transform transforms[];};
...
struct Camera {
	...
	int num_samples;
	float iso;};
layout(std430, binding = 2) buffer Cameras {Camera cameras[];};
...
void triangleIntersection(int triangle_id) {
	...
	closer_intersect.material_index = procedural_mesh.material_index;
...
float intersectRayCubeFull(vec3 cubeMin, vec3 cubeMax){
	...
	float result = max(0.0,dist.x);
	if(dist.y - result < 0.1) intersection_count_box += 2;
	return result;
}
...
const float blue_color =   mix(650.0,purple_color,spector_color_size);
const float green_color =  mix(565.0,blue_color,spector_color_size);
const float red_color =    mix(405.0,green_color,spector_color_size);

const vec4 spectrum[4] = vec4[](
	vec4(0.1, 0.05, 0.3333, purple_color),

	vec4(0.0, 0.0, 0.3333, blue_color),

	vec4(0.0, 0.45, 0.0, green_color),

	vec4(0.9, 0.0, 0.0, red_color)
);
vec4 current_ray_color = vec4(1,1,1,green_color);
vec2 current_ray_color_damph = vec2(green_color*green_color, 1.0/green_color);

#define light_speed 3.33564095e-9

float calculateRefractiveIndex(float n1_base, float n2_base) {
	return n1_base / (n2_base+sqrt(
		n2_base * 2166.6 * current_ray_color_damph.x * 
		current_ray_color_damph.y * light_speed)
	);
}
...
void SelectRayDispersion(int index){
	...
	int indexB = indexA+1;
	current_ray_color = mix(
		spectrum[indexA],
		spectrum[indexB],
		RandomUniform());
	current_ray_color_damph.x = current_ray_color.a * current_ray_color.a;
	current_ray_color_damph.y = 1.0 / current_ray_color.a;
}
...
bool NewRay(float alpha_chanael, float smoothness){
	...
	if(alpha_chanael>=1.0){
		...
		if(closer_intersect.material.inside_transition_prob.x<RandomUniform()){
			direction = mix(
			RandomSphereDirection() - closer_intersect.intersect_info.normal, 
			refract(global_ray.rd, closer_intersect.intersect_info.normal, calculateRefractiveIndex(n1, n2)),
			smoothness);
		}
		else{
			direction = mix(
			RandomSphereDirection() - closer_intersect.intersect_info.normal, 
			refract(global_ray.rd, closer_intersect.intersect_info.normal, calculateRefractiveIndex(n1, n2)),
			min((closer_intersect.material.inside_smoothness.z<RandomUniform()?
			closer_intersect.material.inside_smoothness.x:
			closer_intersect.material.inside_smoothness.y),1.0));
		}
		float len = length(direction);
		if(len > SHIFT) global_ray.rd = direction/len;
		else global_ray.rd = -closer_intersect.intersect_info.normal;
...
vec3 render(){
	...
	// return vec3(
	// 	intersection_count_triangle/TRIANGLE_HIT,
	// 	0,
	// 	intersection_count_box/BOX_HIT
	// );
	//return closer_intersect.material.outside_color1.rgb * (1.0-closer_intersect.intersect_info.is_inside);
	...
	int bounce_count = (RENDERING_RUN ? global_camera.max_bounce_count : 6);
	for(int i = 0; i < bounce_count; i++) {
		if(!closer_intersect.is_intersect) break;


		correlation = (1.0-max(0.0,dot(
			closer_intersect.intersect_info.normal,
			-global_ray.rd
		)))*closer_intersect.material.outside_smoothness.w;
		probability = correlation+closer_intersect.material.outside_smoothness.z<RandomUniform();
		color = (probability?
		...
		if(current_scatter_index==-1){
			...
			if(is_refract && closer_intersect.intersect_info.is_inside==0){
				scatter_stack[scatter_depth] = closer_intersect.material_index;
				scatter_depth+=1;
			}
			if(length(ray_color) <= SHIFT) break;
			UpdateCloserIntersect();
				...
				material = materials[current_scatter_index];
	...
	for(int i = 0; i < CAMERAS_COUNT; i++) {
		...
		if(RENDERING_RUN){
			for(int i = 0; i < global_camera.num_samples; i++) {
				...
				vec2 rand = Random2DCircle()*0.0025;
				global_ray.ro = ro;
				global_ray.rd = rd;
				global_ray.ro += (
