from ..GpuResourceSystem import Shader, Mesh, Texture2D, Texture2DFloat
...
class Assets(TypedDict):
	...
	final_display_texture: Texture2DFloat
...
class AssetsEngineSystem:
	...
	def WindowInitialization(cls, window_id: int) -> None:
		...
		cls.__ASSETS[window_id] = {
			...
			'UV_1k_texture': Texture2D().LoadToRamFromPath(rf"{ASSETS_PATH}\Images\UV_1k.jpg").LoadToGpu(),
			'plane_mesh': Mesh().LoadToRamFromMeshesData([plane]).LoadToGpu(),
			'error_mesh': Mesh().LoadToRamFromPath(rf"{ASSETS_PATH}\Models\ERROR.obj", False).LoadToGpu(),
		}
