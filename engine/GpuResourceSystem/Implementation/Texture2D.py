...
from ...ApiGraphics import GetDataTexture2D
...
from numpy import uint32, ndarray, dtype, uint8, zeros, uint64
...
class Texture2D:
	...
	def OpenTextureForAll(self) -> 'Texture2D':
	...
	def CloseTextureForAll(self) -> 'Texture2D':
		if(not self.__THREAD_LOAD_RAM['ready']):
	...
	def GetData(self) -> ndarray:
		return self.__DATA

	def GetActualData(self) -> ndarray:
		self.__DATA = GetDataTexture2D(self.__OBJECT)
