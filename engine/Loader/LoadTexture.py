...
from numpy import ndarray, dtype, uint8, array, flipud, zeros
...
class Image2D:
	data: ndarray[Tuple[int,int], dtype[uint8]]
	def __init__(self, data: ndarray[Tuple[int,int], dtype[uint8]]) -> None:
		self.data = data

def ReadImage2D(path_to_image: str, image_data_2d: ImageData2D) -> str:
	...
	try:
		data: ndarray[Tuple[int,int], dtype[uint8]] = zeros((1, 1, 4), dtype=uint8)
