...
class Camera(Component):
	...
	def __init__(self,
		...
		self.__LINKED_TO_WINDOW_SIZE = False
	...
	def __perspective_matrix(self) -> None:
		...
		CameraController.SetStatusChanged(self._WINDOW_ID)
	...
	def SetAspect(self, width: int, height: int) -> None:
		if(self.__LINKED_TO_WINDOW_SIZE): WindowContextSystem.GetCurrentWindow().RemoveCallbackSize(self.__UpdateScreen) # type: ignore
		self.__ASPECT = width / (height+1)
		self.__LINKED_TO_WINDOW_SIZE = False
		(self.__perspective_matrix() if(self.__MOD=='perspective') else self.__orthographic_matrix())
	
	def AspectLinkWithWindow(self) -> None:
		...
		self.__ASPECT = size.x / (size.y+1)
		self.__LINKED_TO_WINDOW_SIZE = True
		(self.__perspective_matrix() if(self.__MOD=='perspective') else self.__orthographic_matrix())
