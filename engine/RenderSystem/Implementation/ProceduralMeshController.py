...
class ProceduralMeshController:

	__OBSLATE: Dict[int, bool] = {}
	__NUMPY_ARRAY_FREE_CELLS: Dict[int, Set[int]] = {}
	__NUMPY_ARRAY: Dict[int, ndarray] = {}
	...
	def WindowInitialization(cls, window_id: int) -> None:
		...
		cls.__NUMPY_ARRAY_FREE_CELLS[window_id] = set(range(SSBO_LIMIT))
		cls.__NUMPY_ARRAY[window_id] = zeros(SSBO_LIMIT, dtype=typedata)
		cls.__LIST_OF_REGISTERED_PROCEDURAL[window_id] = zeros(SSBO_LIMIT, dtype= bool)
		cls.__SSBO[window_id] = CreateSSBOBuffer(SSBO_INDEX, cls.__NUMPY_ARRAY[window_id])
	...
	def WindowTerminate(cls, window_id: int) -> None:
		...
		cls.__NUMPY_ARRAY_FREE_CELLS.pop(window_id, None)
		cls.__NUMPY_ARRAY.pop(window_id, None)
	...
	def GetAllocateNumpy(cls, window_id: int) -> ndarray:
		return cls.__NUMPY_ARRAY[window_id]

	@classmethod
	def AllocateIndex(cls, window_id: int) -> int:
		s = cls.__NUMPY_ARRAY_FREE_CELLS[window_id]
		if(s):
			index = s.pop()
	...
	def DeallocateIndex(cls, index: int, window_id: int) -> None:
		...
		cls.__NUMPY_ARRAY_FREE_CELLS[window_id].add(index)
		cls.__OBSLATE[window_id] = True
		cls.__CURENT_COUNT[window_id] -= 1
	...
	def UpdateBuffer(cls, window_id: int) -> int:
		if(cls.__OBSLATE[window_id]):
			UpdateSSBOBuffer(cls.__SSBO[window_id], cls.__NUMPY_ARRAY[window_id][cls.__LIST_OF_REGISTERED_PROCEDURAL[window_id]])
