	...
	for(int i = 0; i < PROCEDURALS_MESHES_COUNT; i++) {
		...
		while(bvh_alternative_shift>0) {
			...
			if(bvh.start_index>=0){
				...
				if(dist_test.x>dist_test.y){
					if(
						dist_test.x>=0.0 &&
						(closer_intersect.is_intersect?
							dist_test.x<closer_intersect.intersect_info.distance:true)
					){
						bvh_alternative_shift++;
					...
					if(
						dist_test.y>=0.0 &&
						(closer_intersect.is_intersect?
							dist_test.y<closer_intersect.intersect_info.distance:true)
					){
						bvh_alternative_shift++;
